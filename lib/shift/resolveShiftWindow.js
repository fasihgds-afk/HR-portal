...
// ─── Shift Window for a Known Date ───────────────────────────────

/**
 * Build the shift window (with grace) for a specific attendance date.
 * Does NOT hit the database — caller must supply the Shift object.
 *
 * @param {Object} shift      - Shift document (startTime, endTime, crossesMidnight, gracePeriod)
 * @param {string} dateStr    - Attendance date "YYYY-MM-DD" (the shift-start date)
 * @returns {{ shiftStart: Date, shiftEnd: Date, graceStart: Date, graceEnd: Date }}
 */
export function computeShiftWindowForDate(shift, dateStr) {
  const graceMin = shift.gracePeriod ?? 20;
  const graceMs = graceMin * 60_000;

  const shiftStart = buildKarachiDate(dateStr, shift.startTime);

  let shiftEnd;
  if (shift.crossesMidnight) {
    shiftEnd = buildKarachiDate(addDays(dateStr, 1), shift.endTime);
  } else {
    shiftEnd = buildKarachiDate(dateStr, shift.endTime);
  }

  return {
    shiftStart,
    shiftEnd,
    graceStart: new Date(shiftStart.getTime() - graceMs),
    graceEnd:   new Date(shiftEnd.getTime() + graceMs),
  };
}
