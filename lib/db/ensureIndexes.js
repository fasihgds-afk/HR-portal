...
export async function ensureAllIndexes() {
  try {
    ...
    const [
      ...
      import('../../models/BreakLog'),
      import('../../models/SuspiciousLog'),
    ]);
    ...
    const results = await Promise.allSettled([
      ...
      Device.createIndexes().catch(err => {
        if (err.message?.includes('already exists') || err.message?.includes('same name as the requested index')) {
          return Promise.resolve();
        }
        throw err;
      }),
      BreakLog.createIndexes().catch(err => {
        if (err.message?.includes('already exists') || err.message?.includes('same name as the requested index')) {
          return Promise.resolve();
        }
        throw err;
      }),
      SuspiciousLog.createIndexes().catch(err => {
        if (err.message?.includes('already exists') || err.message?.includes('same name as the requested index')) {
          return Promise.resolve();
        }
        throw err;
    ...
    results.forEach((result, index) => {
      const modelNames = ['Employee', 'ShiftAttendance', 'User', 'ViolationRules', 'Shift', 'Department', 'PaidLeaveQuarter', 'LeavePolicy', 'Device', 'BreakLog', 'SuspiciousLog'];
